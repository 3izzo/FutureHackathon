    ...
    try {
      ...
      let stats = await page.evaluate(() => {
        ...
        for (var i = 0; i < termsNo; i++) {
          ...
          term.x = reverseString(reverseString(document.querySelectorAll('.pui-accordion-content')[i].children[0].children[0].children[0].children[0].textContent.split('(')[0].trim()).split(' ')[0]);
